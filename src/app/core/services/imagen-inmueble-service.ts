...
import { URL_IMAGEM_INMUEBLE } from '../environments/globals';
...
export class ImagenInmuebleService {
  ...
  uploadImagenInmueble(formData:FormData, idInmueble:number,):Observable<any>{
    ...
    return this._http.post<any>(`${URL_IMAGEM_INMUEBLE}${idInmueble}`, formData);
  ...
  deleteImagen(idImagen:number):Observable<any>{
    ...
    return this._http.delete<any>(`${URL_IMAGEM_INMUEBLE}${idImagen}`);
